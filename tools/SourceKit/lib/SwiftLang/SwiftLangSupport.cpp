...
SwiftLangSupport::SwiftLangSupport(SourceKit::Context &SKCtx)
  ...
  setFileSystemProvider("directly-passed-vfs", llvm::make_unique<DirectlyPassedFileSystemProvider>());
}
...
void SwiftLangSupport::codeComplete(
  ...
  auto *provider = static_cast<DirectlyPassedFileSystemProvider *>(getFileSystemProvider(vfsOptions.name));
  assert(provider);
  auto options = provider->addFileSystem(FS);
  vfsOptions.options = llvm::make_unique<DirectlyPassedFileSystemProvider::Options>(options);
